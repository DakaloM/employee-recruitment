      ...
      .map((col) => {
        ...
        if (col.id?.toLowerCase() === 'status') {
          col.cell = ({ row }) => {
            ...
            return (
              <Badge className={statusLookUp[status]} variant={status === ('Approved' || 'Closed') ? 'success' : status === ('Declined' || 'Closed') ? 'destructive' : 'secondary'}>
                {status}
              </Badge>
            );
          };
    ...
    if (actions) {
      cols.push({
        ...
        cell: ({ row }: DataTableRowActionsProps<T>) => {
          return <DataTableRowActions row={row} Edit={actions.Edit} Delete={actions.Delete} View={actions.View} custom={actions.custom} />;
        },
      });
    }
              ...
              <TableRow key={headerGroup.id} className='text-white'>
                {headerGroup.headers.map((header) => {
                  return (
                    <TableHead
                      key={header.id}
                      className="text-2xl bg-gray-100"
                    >
...
export interface DataTableProps<T extends z.AnyZodObject> {
  ...
  actions?: Pick<DataTableRowActionsProps<T>, 'Edit' | 'custom' | 'Delete' | 'View'>;
